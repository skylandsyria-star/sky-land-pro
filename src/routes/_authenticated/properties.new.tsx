...
export const Route = createFileRoute("/_authenticated/properties/new")({
  head: () => ({
    meta: [
      { title: "إضافة عقار — سكاي لاند" },
      { name: "description", content: "إضافة عقار جديد." },
      { property: "og:title", content: "إضافة عقار" },
      { property: "og:description", content: "إضافة عقار جديد." },
    ],
  }),
  component: NewProp,
});
...
function NewProp() {
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      router.navigate({ to: "/properties/$id", params: { id: data.id } });
    } catch (err) {
  ...
  return (
    <div className="space-y-4">
      <h1 className="text-xl font-black">إضافة عقار جديد</h1>
      <form onSubmit={submit} className="card-elevated space-y-3 p-4">
        <F label="عنوان العقار *">
          <input required className="inp" value={form.title} onChange={(e) => upd("title", e.target.value)} placeholder="مثال: شقة 3 غرف في أبو رمانة" />
        </F>
        ...
        <div className="grid grid-cols-3 gap-3">
          <F label="السعر">
            <input type="number" className="inp" value={form.price} onChange={(e) => upd("price", e.target.value)} />
          </F>
          <F label="العملة">
        ...
        <F label="الوصف">
          <textarea rows={3} className="inp" value={form.description} onChange={(e) => upd("description", e.target.value)} />
        </F>
        <F label="ملاحظات داخلية">
